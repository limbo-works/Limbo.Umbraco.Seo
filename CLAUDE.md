...
Two invariants to preserve when touching either side: the UI alias must **differ** from the schema alias (Umbraco's extension registry keys on alias alone, across all extension types — reuse one and the second registration is dropped with an "already registered" console error, so the editor never appears in the data type picker), and `settings.properties` aliases in `manifests.ts` must match the `[ConfigurationField]` aliases in C#.

Because the aliases differ, data types carried over from Umbraco 13 need fixing up: the core v14 upgrade only adds a nullable `propertyEditorUiAlias` column and never fills it in, and `DataTypeViewModelMapDefinition` then reports `EditorUiAlias ?? EditorAlias` to the backoffice — which resolves to nothing. `Migrations/UpdatePropertyEditorUiAliases.cs`, wired up through the auto-discovered `SeoMigrationPlan`, rewrites those rows. `SitemapFrequencyEditor` intentionally exposes *no* configuration editor even though `SitemapFrequencyConfiguration` exists — `UseNullable` is not honoured by the value converter, so wiring it up would surface a dead setting.
